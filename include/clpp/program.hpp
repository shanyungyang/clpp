...
class Program {
    public:
		...
		/**
		  	\param device		The device where the program is going to
								execute.
            \param options  	Additional compiler options for this program.
		 */
		void build(Device device, const char* options = NULL)
		{
			cl_device_id did = device.id();
            cl_int err = clBuildProgram(id(), 1, &did, options, NULL, NULL);
			if(err == CL_BUILD_PROGRAM_FAILURE)
				throw BuildError(err, __FILE__, __FUNCTION__, __LINE__, getBuildLog(device));

			CLPP_CHECK_ERROR(err);
		}

		/// Build the program on specified devices.
		/**
		  	\param device_list	The list of devices where the program is going
								to execute.
            \param options  	Additional compiler options for this program.
		 */
		void build(const DeviceList& device_list, const char* options = NULL)
		{
			size_t device_num = device_list.size();
            cl_int err = clBuildProgram(id(), device_num, device_list.data(), options, NULL, NULL);
			for(size_t i = 0; i < device_num; ++i)
				if(status(device_list[i]) == CL_BUILD_ERROR)
					throw BuildError(err, __FILE__, __FUNCTION__, __LINE__, getBuildLog(device_list[i]));
		...
		class BuildError : public Error {
			public:
        		BuildError(cl_int code,
						   const char* filename,
						   const char* func,
						   size_t line,
						   const std::string& log)
					: Error(code, filename, func, line), my_log(log)
				{}

				/// Get the compiler error message.
				/**
				  	\return		The compiler error message.
				 */
